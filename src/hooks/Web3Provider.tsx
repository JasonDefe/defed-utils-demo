...
import { getNetworkConfig } from '../utils/utils';
import { hexToAscii } from '../utils/utils';
import { Web3Context } from '../hooks/useWeb3Context';
import { getWallet, WalletType } from '../constants/WalletOptions';
...
export const Web3ContextProvider: React.FC<{ children: ReactElement }> = ({ children }) => {
  ...
  const connectWallet = useCallback(
    async (wallet: WalletType) => {
      setLoading(true);
      try {
        const connector: AbstractConnector = getWallet(wallet);
  ...
  const getTxError = async (txHash: string): Promise<string> => {
    if (provider) {
      ...
      const error = hexToAscii(code.substring(138));
      return error;
    }
